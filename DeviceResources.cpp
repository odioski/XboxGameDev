...
// DeviceResources.cpp
//
// Helper class for Xbox One X Scorpio D3D12 device resource management
//
...
namespace
{
    ...
    inline long ComputeIntersectionArea(long ax1, long ay1, long ax2, long ay2,
...
// Constructor for DeviceResources optimized for Xbox One X Scorpio.
...
// Configures the Direct3D device for Xbox One X Scorpio.
void DeviceResources::CreateDeviceResources()
{
#if defined(_XBOX_ONE) && defined(_TITLE)
    // Xbox One X Scorpio enhanced device creation
    D3D12XBOX_CREATE_DEVICE_PARAMETERS params = {};
    params.Version = D3D12_SDK_VERSION;

#ifdef _DEBUG
    params.ProcessDebugFlags = D3D12XBOX_PROCESS_DEBUG_FLAG_DEBUG_LAYER_ENABLED;
    ...
    {
        OutputDebugStringA("ERROR: Running incorrect Xbox One library version\n");
    }
#endif
    ThrowIfFailed(hr);
    ...
    m_rtvDescriptorHeap->SetName(L"DeviceResources RTV Heap");
    ...
    if (m_depthBufferFormat != DXGI_FORMAT_UNKNOWN)
    {
        ...
        ThrowIfFailed(m_d3dDevice->CreateDescriptorHeap(&dsvHeapDesc, IID_PPV_ARGS(&m_dsvDescriptorHeap)));

        m_dsvDescriptorHeap->SetName(L"DeviceResources DSV Heap");
    }
...
// These resources need to be recreated every time the window size is changed for Xbox One X.
void DeviceResources::CreateWindowSizeDependentResources()
{
    ...
    // Determine the render target size in pixels optimized for Xbox One X 4K.
    ...
    {
        // Create a descriptor for the swap chain optimized for Xbox One X.
    ...
    {
        // Xbox One X supports HDR10
#if defined(_XBOX_ONE) && defined(_TITLE)
        // Set HDR metadata for Xbox One X
        ThrowIfFailed(m_swapChain->SetColorSpace1(DXGI_COLOR_SPACE_RGB_FULL_G2084_NONE_P2020));
#else
        // Check for HDR support on PC
        ComPtr<IDXGISwapChain4> swapChain4;
        if (SUCCEEDED(m_swapChain.As(&swapChain4)))
        {
            ComPtr<IDXGIOutput> output;
            ComPtr<IDXGIOutput6> output6;
            if (SUCCEEDED(swapChain4->GetContainingOutput(&output)) && SUCCEEDED(output.As(&output6)))
            {
                ...
                {
                    ThrowIfFailed(swapChain4->SetColorSpace1(DXGI_COLOR_SPACE_RGB_FULL_G2084_NONE_P2020));
                }
            }
        }
#endif
    ...
    // Set the 3D rendering viewport to target the entire window optimized for Xbox One X.
...
// Set the window associated with the DeviceResources.
...
// This method determines whether the window size has changed for Xbox One X adaptation.
bool DeviceResources::WindowSizeChanged(int width, int height)
{
        ...
        && newRc.bottom == m_outputSize.bottom)
    {
        ...
        if (m_options & c_EnableHDR)
        {
            // Xbox One X supports HDR10 automatically
        }
...
// Prepare the command list and render target for rendering optimized for Xbox One X.
void DeviceResources::Prepare(D3D12_RESOURCE_STATES beforeState, D3D12_RESOURCE_STATES afterState)
{
    ...
    if (beforeState != afterState)
    {
        ...
        auto barrier = CD3DX12_RESOURCE_BARRIER::Transition(m_renderTargets[m_backBufferIndex].Get(), beforeState, afterState);
        m_commandList->ResourceBarrier(1, &barrier);
    }
}

// Present the contents of the swap chain to the screen optimized for Xbox One X.
void DeviceResources::Present(D3D12_RESOURCE_STATES beforeState)
{
    if (beforeState != D3D12_RESOURCE_STATE_PRESENT)
    {
        // Transition the render target to the state that allows it to be presented to the display.
        auto barrier = CD3DX12_RESOURCE_BARRIER::Transition(m_renderTargets[m_backBufferIndex].Get(), beforeState, D3D12_RESOURCE_STATE_PRESENT);
        m_commandList->ResourceBarrier(1, &barrier);
    }
    ...
    else
    {
        ...
        hr = m_swapChain->Present((m_options & c_EnableVSync) ? 1 : 0, 0);
    }
